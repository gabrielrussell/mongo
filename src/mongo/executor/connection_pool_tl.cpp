...
void TLTypeFactory::shutdown() {
    ...
    LOGV2(21608, "Killing all outstanding egress activity.");
...
void TLTimer::setTimeout(Milliseconds timeoutVal, TimeoutCallback cb) {
    ...
    if (inShutdown()) {
        LOGV2_DEBUG(21609, 2, "Skipping timeout due to impending shutdown.");
        return;
    }
...
void TLConnection::setup(Milliseconds timeout, SetupCallback cb) {
    ...
    LOGV2_DEBUG(21611, 2, "Finished connection setup.");
}
