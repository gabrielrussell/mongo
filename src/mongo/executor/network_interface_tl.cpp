...
NetworkInterfaceTL::NetworkInterfaceTL(std::string instanceName,
    ...
    if (!_tl) {
        LOGV2_WARNING(21627, "No TransportLayer configured during NetworkInterface startup");
...
void NetworkInterfaceTL::_run() {
    LOGV2_DEBUG(21618, 2, "The NetworkInterfaceTL reactor thread is spinning up");
    _reactor->run();
    ...
    LOGV2_DEBUG(21619, 2, "NetworkInterfaceTL shutdown successfully");
}
...
void NetworkInterfaceTL::shutdown() {
    ...
    LOGV2_DEBUG(21620, 2, "Shutting down network interface.");
    _reactor->stop();
...
void NetworkInterfaceTL::CommandState::setTimer() {
    ...
    timer->waitUntil(deadline, baton).getAsync([this, anchor = shared_from_this()](Status status) {
        ...
        LOGV2_DEBUG(21621, 2, "{message}", "message"_attr = message);
...
Status NetworkInterfaceTL::startCommand(const TaskExecutor::CallbackHandle& cbHandle,
    ...
    if (MONGO_unlikely(networkInterfaceDiscardCommandsBeforeAcquireConn.shouldFail())) {
        LOGV2(21624, "Discarding command due to failpoint before acquireConn");
        return Status::OK();
    }
