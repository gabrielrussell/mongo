...
void ThreadPoolMock::startup() {
    LOGV2_DEBUG(21628, 1, "Starting pool");
    ...
    _worker = stdx::thread([this] {
        ...
        LOGV2_DEBUG(21629, 1, "Starting to consume tasks");
        while (!_joining) {
            if (_tasks.empty()) {
                lk.unlock();
                _net->waitForWork();
        ...
        LOGV2_DEBUG(21630, 1, "Done consuming tasks");
    });
}
...
void ThreadPoolMock::_shutdown(stdx::unique_lock<Latch>& lk) {
    LOGV2_DEBUG(21631, 1, "Shutting down pool");
...
void ThreadPoolMock::_join(stdx::unique_lock<Latch>& lk) {
    LOGV2_DEBUG(21632, 1, "Joining pool");
