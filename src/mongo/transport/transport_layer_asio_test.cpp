...
public:
    void startSession(transport::SessionHandle session) override {
        ...
        LOGV2(21750, "started session");
        _cv.notify_one();
    }

    void endAllSessions(transport::Session::TagMask tags) override {
        LOGV2(21751, "end all sessions");
...
public:
    explicit SimpleConnectionThread(int port) : _port(port) {
        _thr = stdx::thread{[&] {
            ...
            s.connect(sa);
            LOGV2(21752, "connection: port {port}", "port"_attr = _port);
            stdx::unique_lock<Latch> lk(_mutex);
            _cv.wait(lk, [&] { return _stop; });
            LOGV2(21753, "connection: Rx stop request");
        }};
    }
    ...
    void stop() {
        ...
        LOGV2(21754, "connection: Tx stop request");
        _cv.notify_one();
        _thr.join();
        LOGV2(21755, "connection: stopped");
    }
...
TEST(TransportLayerASIO, PortZeroConnect) {
    ...
    ASSERT_GT(port, 0);
    LOGV2(21756, "TransportLayerASIO.listenerPort() is {port}", "port"_attr = port);
...
public:
    ...
    bool shutdown(Milliseconds timeout) override {
        LOGV2(21757, "Joining all worker threads");
...
public:
    ...
    void startSession(transport::SessionHandle session) override {
        ...
        startWorkerThread([this, session = std::move(session)]() mutable {
            LOGV2(21759, "waiting for message");
            ...
            if (_mode == kShouldTimeout) {
                ...
                LOGV2(21760, "message timed out");
            } else {
                ASSERT_OK(status);
                LOGV2(21761, "message received okay");
            }
...
public:
    void startSession(transport::SessionHandle session) override {
        LOGV2(21762,
              ...
              "session_remote"_attr = session->remote());
        startWorkerThread([this, session = std::move(session)]() mutable {
            LOGV2(21763, "waiting for message");
            ...
            LOGV2(21764, "timed out successfully");
            ASSERT_OK(sourceMessage());
            notifyComplete();

            LOGV2(21765, "waiting for message without a timeout");
