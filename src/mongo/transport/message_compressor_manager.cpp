...
void MessageCompressorManager::clientBegin(BSONObjBuilder* output) {
    LOGV2_DEBUG(21646, 3, "Starting client-side compression negotiation");
    _negotiated.clear();
    ...
    for (const auto e : _registry->getCompressorNames()) {
        LOGV2_DEBUG(21647, 3, "Offering {e} compressor to server", "e"_attr = e);
        sub.append(e);
    }
    sub.doneFast();
}
...
void MessageCompressorManager::clientFinish(const BSONObj& input) {
    ...
    LOGV2_DEBUG(21648, 3, "Finishing client-side compression negotiation");
...
void MessageCompressorManager::serverNegotiate(const BSONObj& input, BSONObjBuilder* output) {
    ...
    if (elem.eoo()) {
        ...
        if (_negotiated.size() > 0) {
            ...
            LOGV2_DEBUG(21653, 3, "Compression negotiation not requested by client");
        }
        return;
    }
    ...
    if (!theirObj.nFields()) {
        LOGV2_DEBUG(21654, 3, "No compressors provided");
        return;
    }
    ...
    for (const auto& elem : theirObj) {
        ...
        if ((cur = _registry->getCompressor(curName))) {
            LOGV2_DEBUG(
                21655, 3, "{cur_getName} is supported", "cur_getName"_attr = cur->getName());
            _negotiated.push_back(cur);
        } else {  // Otherwise the compressor is not supported and we skip over it.
            LOGV2_DEBUG(21656, 3, "{curName} is not supported", "curName"_attr = curName);
        }
    }
    ...
    if (_negotiated.size() > 0) {
        ...
        LOGV2_DEBUG(21657, 3, "Could not agree on compressor to use");
    }
}
