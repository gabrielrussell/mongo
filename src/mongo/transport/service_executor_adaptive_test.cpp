...
protected:
    ...
    std::unique_ptr<ServiceExecutorAdaptive> makeAndStartExecutor() {
        ...
        LOGV2(21679, "wait for executor to finish starting");
        waitFor.store(1);
...
TEST_F(ServiceExecutorAdaptiveFixture, TestStuckTask) {
    ...
    LOGV2(21680, "Scheduling blocked task");
    waitFor.store(3);
    ...
    LOGV2(21681, "Scheduling task stuck on blocked task");
    ASSERT_OK(exec->schedule(
        notifyCallback, ServiceExecutor::kEmptyFlags, ServiceExecutorTaskName::kSSMProcessMessage));

    LOGV2(21682, "Waiting for second thread to start");
    waitForCallback(1);
    ASSERT_EQ(exec->threadsRunning(), 2);

    LOGV2(21683, "Waiting for unstuck task to run");
    blockedLock.unlock();
    waitForCallback(0);
    ASSERT_EQ(exec->threadsRunning(), 2);

    LOGV2(21684, "Waiting for second thread to idle out");
...
TEST_F(ServiceExecutorAdaptiveFixture, TestStuckThreads) {
    ...
    auto blockedTask = [this, &blockedMutex] {
        LOGV2(21685, "waiting on blocked mutex");
        notifyCallback();
        stdx::unique_lock<Latch> lk(blockedMutex);
        notifyCallback();
    };
    ...
    LOGV2(21686, "Scheduling {tasks} blocked tasks", "tasks"_attr = tasks);
    ...
    LOGV2(21687, "Waiting for executor to start new threads");
    waitForCallback(3);

    LOGV2(21688, "All threads blocked, wait for executor to detect block and start a new thread.");
    ...
    LOGV2(21689, "Waiting for unstuck task to run");
    blockedLock.unlock();
    waitForCallback(0);
}
...
TEST_F(ServiceExecutorAdaptiveFixture, TestRecursion) {
    ...
    task = [this, &task, &exec, &mutex, &cv, &remainingTasks] {
        if (remainingTasks.subtractAndFetch(1) == 0) {
            LOGV2(21690, "Signaling job done");
            cv.notify_one();
            return;
        }

        LOGV2(21691, "Starting task recursively");
        ...
        LOGV2(21692, "Completing task recursively");
    };
...
TEST_F(ServiceExecutorAdaptiveFixture, TestDeferredTasks) {
    ...
    waitFor.store(3);
    LOGV2(21693, "Scheduling a blocking task");
    ASSERT_OK(exec->schedule(
        [this, &blockedMutex] {
            stdx::unique_lock<Latch> lk(blockedMutex);
            notifyCallback();
    ...
    LOGV2(21694, "Scheduling deferred task");
    ASSERT_OK(exec->schedule(notifyCallback,
    ...
    LOGV2(21695, "Scheduling non-deferred task");
    ASSERT_OK(exec->schedule(
        notifyCallback, ServiceExecutor::kEmptyFlags, ServiceExecutorTaskName::kSSMProcessMessage));
