...
SSLConnectionOpenSSL::SSLConnectionOpenSSL(SSL_CTX* context,
    ...
    if (len > 0) {
        ...
        if (toBIO != len) {
            LOGV2_DEBUG(21841, 3, "Failed to write initial network data to the SSL BIO layer");
...
Status stapleOCSPResponse(SSL_CTX* context) {
    ...
    if (!swOCSPContext.isOK()) {
        LOGV2_WARNING(21850, "Could not staple OCSP response to outgoing certificate.");
        return swOCSPContext.getStatus();
    }
    ...
    dispatchRequests(context, std::move(intermediateCerts), ocspContext)
        .getAsync([context](StatusWith<std::pair<Status, UniqueOCSPResponse>> swResponse) {
            if (!swResponse.isOK()) {
                LOGV2_WARNING(21851, "Could not staple OCSP response to outgoing certificate.");
                return;
            }
...
unsigned long long SSLManagerOpenSSL::_convertASN1ToMillis(ASN1_TIME* asn1time) {
    ...
    if (timeError <= 0) {
        LOGV2_ERROR(21859, "ASN1_TIME_print failed or wrote no data.");
        return 0;
    }
    ...
    if (timeError <= 0) {
        LOGV2_ERROR(21860, "BIO_gets call failed to transfer contents to buf");
        return 0;
    }
    ...
    *subjectName = getCertificateSubjectX509Name(x509);
    if (serverCertificateExpirationDate != nullptr) {
        ...
        if (notBeforeMillis == 0) {
            LOGV2_ERROR(21864, "date conversion failed");
            return false;
        }

        unsigned long long notAfterMillis = _convertASN1ToMillis(X509_get_notAfter(x509));
        if (notAfterMillis == 0) {
            LOGV2_ERROR(21865, "date conversion failed");
            return false;
        }

        if ((notBeforeMillis > curTimeMillis64()) || (curTimeMillis64() > notAfterMillis)) {
            LOGV2_FATAL(21883, "The provided SSL certificate is expired or not yet valid.");
...
void SSLManagerOpenSSL::_flushNetworkBIO(SSLConnectionOpenSSL* conn) {
    ...
    while ((wantRead = BIO_ctrl_get_read_request(conn->networkBIO)) > 0) {
        ...
        if (toBIO != numRead) {
            LOGV2_DEBUG(21846, 3, "Failed to write network data to the SSL BIO layer");
...
Future<SSLPeerInfo> SSLManagerOpenSSL::parseAndValidatePeerCertificate(
    ...
    if (nullptr == peerCert) {  // no certificate presented by peer
        if (_weakValidation) {
            ...
            if (!_suppressNoCertificateWarning) {
                LOGV2_WARNING(21852, "no SSL certificate provided by peer");
            }
            return SSLPeerInfo(sni);
        } else {
            auto msg = "no SSL certificate provided by peer; connection rejected";
            LOGV2_ERROR(21873, "{msg}", "msg"_attr = msg);
    ...
    if (result != X509_V_OK) {
        if (_allowInvalidCertificates) {
            ...
            msg << "SSL peer certificate validation failed: "
                << X509_verify_cert_error_string(result);
            LOGV2_ERROR(21874, "{msg_ss_str}", "msg_ss_str"_attr = msg.ss.str());
            return Status(ErrorCodes::SSLHandshakeFailed, msg);
        }
    }
    ...
    if (remoteHost.empty()) {
        ...
        if (_sslConfiguration.serverSubjectName() == peerSubject) {
            LOGV2_WARNING(21854, "Client connecting with server's own TLS certificate");
        }
    ...
    if (!sanMatch && !cnMatch) {
        ...
        if (_allowInvalidCertificates || _allowInvalidHostnames || isUnixDomainSocket(remoteHost)) {
            LOGV2_WARNING(21856, "{msg}", "msg"_attr = msg);
        } else {
            LOGV2_ERROR(21875, "{msg}", "msg"_attr = msg);
...
void SSLManagerOpenSSL::_handleSSLError(SSLConnectionOpenSSL* conn, int ret) {
    ...
    switch (code) {
        ...
        case SSL_ERROR_WANT_WRITE:
            ...
            LOGV2_ERROR(
                21876, "SSL: {code}, possibly timed out during connect", "code"_attr = code);
            break;

        case SSL_ERROR_ZERO_RETURN:
            // TODO: Check if we can avoid throwing an exception for this condition
            // If so, change error() back to LOG(3)
            LOGV2_ERROR(21877, "SSL network connection closed");
            break;
        case SSL_ERROR_SYSCALL:
            ...
            if (err != 0) {
                ...
                LOGV2_ERROR(21879, "Unexpected EOF encountered during SSL communication");
            } else {
                LOGV2_ERROR(21880,
        ...
        default:
            LOGV2_ERROR(21882, "unrecognized SSL error");
            break;
    }
