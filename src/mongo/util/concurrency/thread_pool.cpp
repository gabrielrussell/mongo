...
ThreadPool::~ThreadPool() {
    ...
    if (shutdownComplete != _state) {
        LOGV2_FATAL(21781, "Failed to shutdown pool during destruction");
        fassertFailed(28704);
    }
...
void ThreadPool::_workerThreadBody(ThreadPool* pool, const std::string& threadName) noexcept {
    ...
    LOGV2_DEBUG(21769, 1, "starting thread in pool {poolName}", "poolName"_attr = poolName);
    pool->_consumeTasks();
    ...
    LOGV2_DEBUG(21770, 1, "shutting down thread in pool {poolName}", "poolName"_attr = poolName);
}
