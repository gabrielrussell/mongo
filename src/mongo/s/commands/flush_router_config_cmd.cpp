...
public:
    ...
    bool run(OperationContext* opCtx,
        ...
        if (argumentElem.isNumber() || argumentElem.isBoolean()) {
            LOGV2(22324, "Routing metadata flushed for all databases");
            catalogCache->purgeAllDatabases();
        } else {
            const auto ns = argumentElem.checkAndGetStringData();
            if (nsIsDbOnly(ns)) {
                LOGV2(22325, "Routing metadata flushed for database {ns}", "ns"_attr = ns);
                catalogCache->purgeDatabase(ns);
            } else {
                const NamespaceString nss(ns);
                LOGV2(22326, "Routing metadata flushed for collection {nss}", "nss"_attr = nss);
