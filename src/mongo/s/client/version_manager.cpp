...
bool initShardVersionEmptyNS(OperationContext* opCtx, DBClientBase* conn_in) {
    try {
        ...
        LOGV2_DEBUG(22304, 3, "initial sharding result : {result}", "result"_attr = result);
...
bool checkShardVersion(OperationContext* opCtx,
    ...
    if (setShardVersion(opCtx,
                        ...
                        authoritative,
                        result)) {
        LOGV2_DEBUG(22307, 1, "      setShardVersion success: {result}", "result"_attr = result);
    ...
    LOGV2_DEBUG(22308, 1, "       setShardVersion failed!\n{result}", "result"_attr = result);
    ...
    string errmsg = str::stream() << "setShardVersion failed shard: " << shard->toString() << " "
                                  << result;
    LOGV2(22309, "     {errmsg}", "errmsg"_attr = errmsg);
