...
void ShardRegistry::shutdown() {
    if (_executor && !_isShutdown) {
        LOGV2_DEBUG(22286, 1, "Shutting down task executor for reloading shard registry");
        _executor->shutdown();
        _executor->join();
        _isShutdown = true;
    }
...
void ShardRegistry::startup(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(22287, 1, "Starting up task executor for periodic reloading of ShardRegistry");
    _executor->startup();
    ...
    if (status.getStatus() == ErrorCodes::ShutdownInProgress) {
        LOGV2_DEBUG(22288, 1, "Cant schedule Shard Registry reload. Executor shutdown in progress");
        return;
    }
...
void ShardRegistry::_internalReload(const CallbackArgs& cbArgs) {
    LOGV2_DEBUG(22289, 1, "Reloading shardRegistry");
    ...
    if (status.getStatus() == ErrorCodes::ShutdownInProgress) {
        LOGV2_DEBUG(22291, 1, "Cant schedule ShardRegistry reload. Executor shutdown in progress");
        return;
    }
