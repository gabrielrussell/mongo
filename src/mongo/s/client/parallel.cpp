        ...
        try {
            if (mdata.initialized) {
                ...
                if (primary && !state->primary)
                    LOGV2_WARNING(22260, "Collection becoming unsharded detected");
                if (manager && !state->manager)
                    LOGV2_WARNING(22261, "Collection becoming sharded detected");
                if (primary && state->primary && primary != state->primary)
                    LOGV2_WARNING(22262, "Weird shift of primary detected");
...
void ParallelSortClusteredCursor::finishInit(OperationContext* opCtx) {
    ...
    for (auto i = _cursorMap.begin(); i != _cursorMap.end();) {
        ...
        // Erase empty stuff
        if (!mdata.pcState) {
            LOGV2(22252, "PCursor erasing empty state {mdata}", "mdata"_attr = mdata.toBSON());
            _cursorMap.erase(i++);
            continue;
        } else {
            ++i;
...
void ParallelSortClusteredCursor::_oldInit(OperationContext* opCtx) {
    ...
    do {
        ...
        if (!firstPass) {
            ...
            for (set<int>::const_iterator it = retryQueries.begin(); it != retryQueries.end();
                 ++it) {
                LOGV2(22254, "{serverHosts_it}, ", "serverHosts_it"_attr = serverHosts[*it]);
            }
            LOGV2(22255,
    ...
    if (socketExs.size() > 0 || staleConfigExs.size() > 0 || otherExs.size() > 0) {
        ...
        if (throwException && staleConfigExs.size() > 0) {
            ...
            LOGV2_WARNING(22276, "{errMsg_str}", "errMsg_str"_attr = redact(errMsg.str()));
        }
    }
        ...
        if (initialized && !errored) {
            ...
            if (!finished && pcState->conn->ok()) {
                try {
                    ...
                    LOGV2_WARNING(22277, "exception closing cursor");
                } catch (...) {
                    LOGV2_WARNING(22278, "unknown exception closing cursor");
                }
            }
        }
