    ...
    void onConfirmedSet(const State& state) noexcept final {
        ...
        auto fun = [serviceContext = _serviceContext, connStr](auto args) {
            ...
            try {
                ...
                LOGV2(22347, "Unable to update sharding state due to {e}", "e"_attr = e);
            }
        };
...
ExitCode runMongosServer(ServiceContext* serviceContext) {
    ...
    auto res = tl->setup();
    if (!res.isOK()) {
        LOGV2_ERROR(22356, "Failed to set up listener: {res}", "res"_attr = res);
...

    {
        ...
        if (!status.isOK()) {
            if (status == ErrorCodes::CallbackCanceled) {
                ...
                LOGV2(22350, "Shutdown called before mongos finished starting up");
                return EXIT_CLEAN;
            }
            LOGV2_ERROR(
                22357, "Error initializing sharding system: {status}", "status"_attr = status);
    ...
    if (!status.isOK()) {
        LOGV2_ERROR(
            22358, "Initializing authorization data failed: {status}", "status"_attr = status);
        return EXIT_SHARDING_ERROR;
    }
    ...
    if (!status.isOK()) {
        LOGV2_ERROR(
            22361, "Failed to start the transport layer: {status}", "status"_attr = redact(status));
        return EXIT_NET_ERROR;
    }
...
ExitCode mongoSMain(int argc, char* argv[], char** envp) {
    ...
    try {
        ...
        LOGV2_ERROR(22362, "uncaught DBException in mongos main: {e}", "e"_attr = redact(e));
        ...
        LOGV2_ERROR(22364, "uncaught unknown exception in mongos main");
        return EXIT_UNCAUGHT;
    }
}
