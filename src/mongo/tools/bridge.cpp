...
public:
    Status runBridgeCommand(StringData cmdName, BSONObj cmdObj) {
        ...
        LOGV2(23488, "Processing bridge command: {cmdName}", "cmdName"_attr = cmdName);
...
int bridgeMain(int argc, char** argv, char** envp) {
    ...
    if (!tl->setup().isOK()) {
        LOGV2(23494, "Error setting up transport layer");
        return EXIT_NET_ERROR;
    }

    if (!tl->start().isOK()) {
        LOGV2(23495, "Error starting transport layer");
        return EXIT_NET_ERROR;
    }
