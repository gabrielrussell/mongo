...
void assertEquals(const std::string& json,
    ...
    if (bad) {
        ...
        LOGV2(23997, "{expected_hexDump}", "expected_hexDump"_attr = expected.hexDump());
        LOGV2(23998, "{actual_hexDump}", "actual_hexDump"_attr = actual.hexDump());
        LOGV2(23999, "{msg}", "msg"_attr = msg);
        LOGV2(24000, "orig json:{json}", "json"_attr = json);
    }
    ASSERT(!bad);
}
