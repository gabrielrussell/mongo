...
public:
    void run() {
        ...
        LOGV2(24003, "Present TS: {presentTs}", "presentTs"_attr = presentTs);
        LOGV2(24004, "Before transaction TS: {beforeTxnTs}", "beforeTxnTs"_attr = beforeTxnTs);
        LOGV2(24005, "Commit entry TS: {commitEntryTs}", "commitEntryTs"_attr = commitEntryTs);
...
public:
    void run() {
        ...
        {
            LOGV2(24006, "inserting {badDoc1}", "badDoc1"_attr = badDoc1);
            WriteUnitOfWork wuow(_opCtx);
            insertDocument(autoColl.getCollection(),
        ...
        {
            LOGV2(24007, "inserting {badDoc2}", "badDoc2"_attr = badDoc2);
            WriteUnitOfWork wuow(_opCtx);
            insertDocument(autoColl.getCollection(),
...

        {
            ...
            // indexable.
            LOGV2(24008, "attempting to insert {badDoc3}", "badDoc3"_attr = badDoc3);
            WriteUnitOfWork wuow(_opCtx);
            ASSERT_THROWS_CODE(
...
public:
    ...
    void logTimestamps() const {
        LOGV2(24009, "Present TS: {presentTs}", "presentTs"_attr = presentTs);
        LOGV2(24010, "Before transaction TS: {beforeTxnTs}", "beforeTxnTs"_attr = beforeTxnTs);
        LOGV2(24011, "Commit entry TS: {commitEntryTs}", "commitEntryTs"_attr = commitEntryTs);
    }
...
public:
    ...
    void run() {
        auto txnParticipant = TransactionParticipant::get(_opCtx);
        ASSERT(txnParticipant);
        LOGV2(24012, "PrepareTS: {prepareEntryTs}", "prepareEntryTs"_attr = prepareEntryTs);
        logTimestamps();
...
public:
    ...
    void run() {
        auto txnParticipant = TransactionParticipant::get(_opCtx);
        ASSERT(txnParticipant);
        LOGV2(24013, "PrepareTS: {prepareEntryTs}", "prepareEntryTs"_attr = prepareEntryTs);
        LOGV2(24014, "AbortTS: {abortEntryTs}", "abortEntryTs"_attr = abortEntryTs);
...
public:
    ...
    void run() {
        ...
        commitEntryTs = currentTime.addTicks(2).asTimestamp();
        LOGV2(24015, "Prepare TS: {prepareTs}", "prepareTs"_attr = prepareTs);
        logTimestamps();
...
public:
    ...
    void run() {
        ...
        LOGV2(24016, "Prepare TS: {prepareTs}", "prepareTs"_attr = prepareTs);
        logTimestamps();
