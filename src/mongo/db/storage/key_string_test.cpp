...
TEST_F(KeyStringBuilderTest, DecimalNumbers) {
    if (version == KeyString::Version::V0) {
        LOGV2(21347, "not testing DecimalNumbers for KeyStringBuilder V0");
        return;
    }
    ...
    for (size_t k = 0; k < orderings.size(); k++) {
        futures.push_back(
            stdx::async(stdx::launch::async, [k, version, elementsOrig, orderings, debug] {
                ...
                if (debug)
                    LOGV2(21348, "ordering: {orderObj}", "orderObj"_attr = orderObj);
...
unsigned newSeed() {
    ...
    LOGV2(21351, "Initializing random number generator using seed {seed}", "seed"_attr = seed);
    return seed;
};
