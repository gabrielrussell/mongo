...
bool OplogCapMaintainerThread::_deleteExcessDocuments() {
    if (!getGlobalServiceContext()->getStorageEngine()) {
        LOGV2_DEBUG(21359, 2, "OplogCapMaintainerThread: no global storage engine yet");
        return false;
    }
    ...
    try {
        ...
        NamespaceString oplogNss = NamespaceString::kRsOplogNamespace;
        {
            ...
            if (!db) {
                LOGV2_DEBUG(21360, 2, "no local database yet");
                return false;
            }
            ...
            if (!collection) {
                LOGV2_DEBUG(21361, 2, "no collection {oplogNss}", "oplogNss"_attr = oplogNss);
                return false;
            }
        ...
        LOGV2_FATAL(21362, "error in OplogCapMaintainerThread: {e_what}", "e_what"_attr = e.what());
