...
void WiredTigerRecordStore::OplogStones::_calculateStonesByScanning(OperationContext* opCtx) {
    ...
    LOGV2(21503, "Scanning the oplog to determine where to place markers for truncation");
...
void WiredTigerRecordStore::OplogStones::_calculateStonesBySampling(OperationContext* opCtx,
    ...
    LOGV2(21505, "Sampling the oplog to determine where to place markers for truncation");
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        if (!record) {
            // This shouldn't really happen unless the size storer values are far off from reality.
            // The collection is probably empty, but fall back to scanning the oplog just in case.
            LOGV2(21510, "Failed to get enough random samples, falling back to scanning the oplog");
            _calculateStonesByScanning(opCtx);
            return;
        }
    ...
    LOGV2(21512, "Oplog sampling complete");
...
WiredTigerRecordStore::~WiredTigerRecordStore() {
    ...
    if (!isTemp()) {
        LOGV2_DEBUG(21514, 1, "~WiredTigerRecordStore for: {ns}", "ns"_attr = ns());
    } else {
        LOGV2_DEBUG(21515,
                    1,
    ...
    try {
        ...
        if (docsRemoved > 0) {
            ...
            if (ret == ENOENT || ret == WT_NOTFOUND) {
                ...
                LOGV2(21516, "Soft failure truncating capped collection. Will try again later.");
                docsRemoved = 0;
            } else {
                invariantWTOK(ret);
        ...
        LOGV2(21517, "got conflict truncating capped, ignoring");
        return 0;
    } catch (...) {
        opCtx->releaseRecoveryUnit();
        opCtx->setRecoveryUnit(std::unique_ptr<RecoveryUnit>(realRecoveryUnit), realRUstate);
...
void WiredTigerRecordStore::reclaimOplog(OperationContext* opCtx, Timestamp mayTruncateUpTo) {
    ...
    while (auto stone = _oplogStones->peekOldestStoneIfNeeded()) {
        ...
        try {
            ...
            LOGV2_DEBUG(
                21519, 1, "Caught WriteConflictException while truncating oplog entries, retrying");
        }
    }
...
Status WiredTigerRecordStore::_insertRecords(OperationContext* opCtx,
    ...
    for (size_t i = 0; i < nRecords; i++) {
        ...
        if (!ts.isNull()) {
            LOGV2_DEBUG(21522, 4, "inserting record with timestamp {ts}", "ts"_attr = ts);
...
void WiredTigerRecordStore::validate(OperationContext* opCtx,
    ...
    if (err == EBUSY) {
        ...
        LOGV2_WARNING(21527, "{msg}", "msg"_attr = msg);
    ...
    LOGV2_ERROR(21528, "{msg}", "msg"_attr = msg);
...
void WiredTigerRecordStore::cappedTruncateAfter(OperationContext* opCtx,
    ...
    if (_isOplog) {
        ...
        LOGV2_DEBUG(21524, 1, "truncation new read timestamp: {truncTs}", "truncTs"_attr = truncTs);
    }
