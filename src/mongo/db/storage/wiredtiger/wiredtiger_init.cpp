...
public:
    ...
    virtual StorageEngine* create(const StorageGlobalParams& params,
        ...
        if (lockFile && lockFile->createdByUncleanShutdown()) {
            LOGV2_WARNING(21421, "Recovering data from the last clean checkpoint.");
        }
...
#define EXT4_SUPER_MAGIC 0xEF53
        {
            ...
            if (ret == 0 && fs_stats.f_type == EXT4_SUPER_MAGIC) {
                LOGV2_OPTIONS(21415, {logv2::LogTag::kStartupWarnings}, "");
                LOGV2_OPTIONS(
                    21416,
        ...
        if (p.supported()) {
            if (cacheMB > memoryThresholdPercentage * p.getMemSizeMB()) {
                LOGV2_OPTIONS(21418, {logv2::LogTag::kStartupWarnings}, "");
                LOGV2_OPTIONS(21419,
                              {logv2::LogTag::kStartupWarnings},
