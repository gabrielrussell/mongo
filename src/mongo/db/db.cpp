...
ExitCode _initAndListen(int listenPort) {
    ...
    if (kDebugBuild)
        LOGV2_OPTIONS(
            20873, {logComponentV1toV2(LogComponent::kControl)}, "DEBUG build (which is slower)");
    ...
    if (!storageGlobalParams.repair) {
        ...
        auto res = tl->setup();
        if (!res.isOK()) {
            LOGV2_ERROR(20908, "Failed to set up listener: {res}", "res"_attr = res);
    ...
    if (gFlowControlEnabled.load()) {
        LOGV2(20876, "Flow Control is enabled on this deployment.");
    }

    if (storageGlobalParams.upgrade) {
        LOGV2(20877, "finished checking dbs");
        exitCleanly(EXIT_CLEAN);
    }
    ...
    if (globalAuthzManager->shouldValidateAuthSchemaOnStartup()) {
        ...
        if (!status.isOK()) {
            LOGV2(20878, "{status}", "status"_attr = redact(status));
        ...
        LOGV2_ERROR(20909, "Auth must be disabled when starting without auth schema validation");
        exitCleanly(EXIT_BADOPTIONS);
    } else {
        // If authSchemaValidation is disabled and server is running without auth,
        // warn the user and continue startup without authSchema metadata checks.
        LOGV2_OPTIONS(20882, {logv2::LogTag::kStartupWarnings}, "");
        LOGV2_OPTIONS(20883,
    ...
    if (!storageGlobalParams.readOnly) {
        ...
        if (getReplSetMemberInStandaloneMode(serviceContext)) {
            LOGV2_OPTIONS(20886, {logv2::LogTag::kStartupWarnings}, "");
            LOGV2_OPTIONS(20887,
                          {logv2::LogTag::kStartupWarnings},
            ...
            LOGV2(20892, "**         ");
            LOGV2(20893, " For more info see http://dochub.mongodb.org/core/ttlcollections");
            LOGV2_OPTIONS(20894, {logv2::LogTag::kStartupWarnings}, "");
        } else {
    ...
    if (!start.isOK()) {
        LOGV2_ERROR(20910, "Failed to start the service executor: {start}", "start"_attr = start);
        return EXIT_NET_ERROR;
    }
    ...
    if (!start.isOK()) {
        LOGV2_ERROR(
            20911, "Failed to start the service entry point: {start}", "start"_attr = start);
        return EXIT_NET_ERROR;
    }
    ...
    if (!storageGlobalParams.repair) {
        ...
        if (!start.isOK()) {
            LOGV2_ERROR(
                20912, "Failed to start the listener: {start}", "start"_attr = start.toString());
            return EXIT_NET_ERROR;
        }
    }
    ...
    if (ntservice::shouldStartService()) {
        ...
        LOGV2(20895, "Service running");
    }
#endif

    if (MONGO_unlikely(shutdownAtStartup.shouldFail())) {
        LOGV2(20896, "starting clean exit via failpoint");
        exitCleanly(EXIT_CLEAN);
    }
...
ExitCode initAndListen(int listenPort) {
    try {
        ...
        LOGV2(20897, "exception in initAndListen: {e}, terminating", "e"_attr = e.toString());
        ...
        LOGV2(20899, "exception in initAndListen int: {n}, terminating", "n"_attr = n);
        return EXIT_UNCAUGHT;
    } catch (...) {
        LOGV2(20900, "exception in initAndListen, terminating");
        return EXIT_UNCAUGHT;
    }
}
...
void shutdownTask(const ShutdownTaskArgs& shutdownArgs) {
    ...
    LOGV2_OPTIONS(20905, {logComponentV1toV2(LogComponent::kControl)}, "now exiting");
