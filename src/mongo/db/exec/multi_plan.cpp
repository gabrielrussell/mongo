...
PlanStage::StageState MultiPlanStage::doWork(WorkingSetID* out) {
    ...
    if (PlanStage::FAILURE == state && hasBackupPlan()) {
        LOGV2_DEBUG(23866, 5, "Best plan errored out switching to backup");
    ...
    if (hasBackupPlan() && PlanStage::ADVANCED == state) {
        LOGV2_DEBUG(23867, 5, "Best plan had a blocking stage, became unblocked");
        _backupPlanIdx = kNoSuchPlan;
    }
...
Status MultiPlanStage::pickBestPlan(PlanYieldPolicy* yieldPolicy) {
    ...
    if (bestSolution->hasBlockingStage && (0 == alreadyProduced.size())) {
        LOGV2_DEBUG(23870, 5, "Winner has blocking stage, looking for backup plan...");
        for (auto&& ix : candidateOrder) {
            if (!_candidates[ix].solution->hasBlockingStage) {
                LOGV2_DEBUG(23871, 5, "Candidate {ix} is backup child", "ix"_attr = ix);
                _backupPlanIdx = ix;
                break;
            }
        }
