...
public:
    ...
    virtual void help(std::stringstream& help) const {
        help << "start logical session ";
    }
    virtual void addRequiredPrivileges(const std::string& dbname,
                                       const BSONObj& cmdObj,
                                       std::vector<Privilege>* out) {
        ActionSet actions;
        actions.addAction(ActionType::startSession); // XXX
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        {
            BSONObjBuilder b(result.subobjStart(""));
            auto client = opCtx->getClient();
            auto userNameItr = AuthorizationSession::get(client)->getAuthenticatedUserNames();
            auto lsCache = client->getServiceContext().getLogicalSessionCache();
            auto lsRecord = LogicalSessionRecord::makeAuthoritativeRecord(LogicalSessionId::gen(),user,uid,time)
            b.done();
        }
        return true;
    }
