        ...
        while (i.moreInCurrentBatch()) {
            ...
            if (!status.isOK()) {
                ...
                if (gSkipCorruptDocumentsWhenCloning.load()) {
                    LOGV2_WARNING(20855, "{ss_ss_str}; skipping", "ss_ss_str"_attr = ss.ss.str());
                    continue;
                }
...
bool Cloner::copyCollection(OperationContext* opCtx,
    ...
    if (!shouldCopyIndexes) {
        LOGV2(
            20849, "ERROR copy collection shouldCopyIndexes not implemented? {ns}", "ns"_attr = ns);
    }
...
StatusWith<std::vector<BSONObj>> Cloner::filterCollectionsForClone(
    ...
    for (auto&& collection : initialCollections) {
        LOGV2_DEBUG(20850, 2, "\t cloner got {collection}", "collection"_attr = collection);
        ...
        if (ns.isSystem()) {
            if (!ns.isLegalClientSystemNS()) {
                LOGV2_DEBUG(20851, 2, "\t\t not cloning because system collection");
                continue;
            }
        }
