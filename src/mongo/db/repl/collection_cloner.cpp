...
void CollectionCloner::runQuery() {
    ...
    if (_resumeSupported) {
        if (_resumeToken) {
            ...
            LOGV2_DEBUG(22772, 1, "Collection cloner will resume the last successful query");
            ...
            LOGV2_DEBUG(22773, 1, "Collection cloner will run a new query");
            query = QUERY("query" << BSONObj() << "$readOnce" << true << "$_requestResumeToken"
                                  << true);
        }
...
void CollectionCloner::handleNextBatch(DBClientCursorBatchIterator& iter) {
    {
        ...
        if (!getSharedData()->getInitialSyncStatus(lk).isOK()) {
            ...
            LOGV2(22775, "{message}", "message"_attr = message);
...
void CollectionCloner::killOldQueryCursor() {
    ...
    LOGV2_DEBUG(22778, 1, "Attempting to kill old remote cursor with id: {id}", "id"_attr = id);
    try {
        getClient()->runCommand(nss.db().toString(), cmdObj, infoObj);
    } catch (...) {
        LOGV2(22779, "Error while trying to kill remote cursor after transient query error");
    }
...
void CollectionCloner::abortNonResumableClone(const Status& status) {
    ...
    LOGV2(22780, "Error during non-resumable clone: {status}", "status"_attr = status);
    std::string message = str::stream()
