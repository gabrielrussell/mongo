...
Future<void> OplogApplier::startup() {
    ...
    auto callback =
        ...
        LOGV2(22863, "Starting oplog application");
        _run(_oplogBuffer);
        LOGV2(22864, "Finished oplog application");
        promise.setWith([] {});
    };
...
void OplogApplier::shutdown() {
    ...
    if (globalFailPointRegistry().find("rsSyncApplyStop")->shouldFail()) {
        LOGV2_FATAL(22866, "Turn off rsSyncApplyStop before attempting clean shutdown");
