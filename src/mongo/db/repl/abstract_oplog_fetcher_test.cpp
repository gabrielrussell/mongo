...
TEST_F(AbstractOplogFetcherTest, OplogFetcherStopsRestartingFetcherIfRestartLimitIsReached) {
    ...
    LOGV2(22677, "processing find request from first fetcher");

    _assertFindCommandTimestampEquals(
        ops[0], processNetworkResponse({makeCursorResponse(1, {ops[0], ops[1], ops[2]})}, true));

    LOGV2(22678, "sending error response to getMore request from first fetcher");
    assertRemoteCommandNameEquals(
        "getMore", processNetworkResponse({ErrorCodes::CappedPositionLost, "fail 1"}, true));

    LOGV2(22679, "sending error response to find request from second fetcher");
    _assertFindCommandTimestampEquals(
        ops[2], processNetworkResponse({ErrorCodes::IllegalOperation, "fail 2"}, true));

    LOGV2(22680, "sending error response to find request from third fetcher");
    _assertFindCommandTimestampEquals(
...
TEST_F(AbstractOplogFetcherTest, OplogFetcherResetsRestartCounterOnSuccessfulFetcherResponse) {
    ...
    LOGV2(22681, "processing find request from first fetcher");

    _assertFindCommandTimestampEquals(
        ops[0], processNetworkResponse({makeCursorResponse(1, {ops[0], ops[1], ops[2]})}, true));

    LOGV2(22682, "sending error response to getMore request from first fetcher");
    assertRemoteCommandNameEquals(
        "getMore", processNetworkResponse({ErrorCodes::CappedPositionLost, "fail 1"}, true));

    LOGV2(22683, "processing find request from second fetcher");
    _assertFindCommandTimestampEquals(
        ops[2], processNetworkResponse({makeCursorResponse(1, {ops[2], ops[3], ops[4]})}, true));

    LOGV2(22684, "sending error response to getMore request from second fetcher");
    assertRemoteCommandNameEquals(
        "getMore", processNetworkResponse({ErrorCodes::IllegalOperation, "fail 2"}, true));

    LOGV2(22685, "sending error response to find request from third fetcher");
    _assertFindCommandTimestampEquals(
        ops[4], processNetworkResponse({ErrorCodes::InternalError, "fail 3"}, true));

    LOGV2(22686, "sending error response to find request from fourth fetcher");
    _assertFindCommandTimestampEquals(
...
TEST_F(AbstractOplogFetcherTest,
    ...
    LOGV2(22687, "processing find request from first fetcher");

    _assertFindCommandTimestampEquals(
        ops[0], processNetworkResponse({makeCursorResponse(1, {ops[0], ops[1], ops[2]})}, true));

    LOGV2(22688, "sending error response to getMore request from first fetcher");
