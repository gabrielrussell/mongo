...
Status RollbackImpl::_transitionToRollback(OperationContext* opCtx) {
    ...
    LOGV2(23230, "transition to ROLLBACK");
    {
        ...
        if (!status.isOK()) {
            ...
            LOGV2(23231, "{status}", "status"_attr = status);
            return status;
        }
    }
    return Status::OK();
...
Status RollbackImpl::_awaitBgIndexCompletion(OperationContext* opCtx) {
    ...
    LOGV2(23232, "Waiting for all background operations to complete before starting rollback");
    for (auto db : dbNames) {
    ...
    LOGV2(23234, "Finished waiting for background operations to complete before rollback");
    return Status::OK();
}
...
void RollbackImpl::_runPhaseFromAbortToReconstructPreparedTxns(
    ...
    if (shouldCreateDataFiles()) {
        ...
        LOGV2(23235, "Not writing rollback files. 'createRollbackDataFiles' set to false.");
    }
...
Status RollbackImpl::_findRecordStoreCounts(OperationContext* opCtx) {
    ...
    LOGV2(23241, "finding record store counts");
...
StatusWith<RollBackLocalOperations::RollbackCommonPoint> RollbackImpl::_findCommonPoint(
    ...
    LOGV2(23243, "finding common point");
...
Status RollbackImpl::_triggerOpObserver(OperationContext* opCtx) {
    ...
    LOGV2(23247, "Triggering the rollback op observer");
...
void RollbackImpl::_transitionFromRollbackToSecondary(OperationContext* opCtx) {
    ...
    LOGV2(23248, "transition to SECONDARY");
...
void RollbackImpl::_summarizeRollback(OperationContext* opCtx) const {
    ...
    if (_rollbackStats.lastLocalWallClockTime &&
        ...
        LOGV2(23258,
              ...
              "firstOpWallClockTimeAfterCommonPoint"_attr = firstOpWallClockTimeAfterCommonPoint);
        LOGV2(23259, "\tdifference in wall clock times: {diff} second(s)", "diff"_attr = diff);
    }
    ...
    for (const auto& sessionId : _observerInfo.rollbackSessionIds) {
        LOGV2(23265, "\t\t{sessionId}", "sessionId"_attr = sessionId);
    }
    LOGV2(23266,
    ...
    for (const auto& nss : _observerInfo.rollbackNamespaces) {
        LOGV2(23267, "\t\t{nss_ns}", "nss_ns"_attr = nss.ns());
    }
    LOGV2(23268,
