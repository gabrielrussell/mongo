...
TEST_F(RollbackImplTest, RollbackKillsNecessaryOperations) {
    ...
    LOGV2(23286, "Both opCtx's marked for kill");
    ...
    LOGV2(23287, "Both opCtx's were interrupted");
...
DEATH_TEST_F(RollbackImplTest,
    ...
    auto status = _rollback->runRollback(_opCtx.get());
    LOGV2(23288, "Mongod did not crash. Status: {status}", "status"_attr = status);
    MONGO_UNREACHABLE;
}
...
DEATH_TEST_F(RollbackImplTest,
    ...
    auto status = _rollback->runRollback(_opCtx.get());
    LOGV2(23289, "mongod did not crash when expected; status: {status}", "status"_attr = status);
}
...
DEATH_TEST_F(RollbackImplTest,
    ...
    auto status = _rollback->runRollback(_opCtx.get());
    LOGV2(23290, "mongod did not crash when expected; status: {status}", "status"_attr = status);
}
...
DEATH_TEST_F(RollbackImplObserverInfoTest,
    ...
    auto status = _rollback->runRollback(_opCtx.get());
    LOGV2(23292, "Mongod did not crash. Status: {status}", "status"_attr = status);
    MONGO_UNREACHABLE;
}
