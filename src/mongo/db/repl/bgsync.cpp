...
void BackgroundSync::_run() {
    ...
    while (!inShutdown()) {
        try {
            ...
            LOGV2_ERROR(22764, "{msg}", "msg"_attr = msg);
            ...
            LOGV2_FATAL(
                22766, "sync producer exception: {e2_what}", "e2_what"_attr = redact(e2.what()));
            fassertFailed(28546);
        }
    }
...
void BackgroundSync::_produce() {
    ...
    if (syncSourceResp.syncSourceStatus == ErrorCodes::OplogStartMissing) {
        ...
        if (_replCoord->getMemberState().primary()) {
            LOGV2_WARNING(22754, "Too stale to catch up.");
            LOGV2(22720,
        ...
        LOGV2_ERROR(22765, "too stale to catch up -- entering maintenance mode");
        LOGV2(22723,
              "Our newest OpTime : {lastOpTimeFetched}",
              "lastOpTimeFetched"_attr = lastOpTimeFetched);
        LOGV2(22724,
              ...
              "syncSourceResp_earliestOpTimeSeen"_attr = syncSourceResp.earliestOpTimeSeen);
        LOGV2(22725, "See http://dochub.mongodb.org/core/resyncingaverystalereplicasetmember");
    ...
    if (_tooStale.swap(false)) {
        LOGV2(22730, "No longer too stale. Able to sync from {source}", "source"_attr = source);
        auto status = _replCoord->setMaintenanceMode(false);
        if (!status.isOK()) {
            LOGV2_WARNING(
                22757, "Failed to leave maintenance mode: {status}", "status"_attr = status);
        }
    }
    ...
    oplogFetcher->join();
    LOGV2_DEBUG(
        22732, 1, "fetcher stopped reading remote oplog on {source}", "source"_attr = source);
    ...
    if (fetcherReturnStatus.code() == ErrorCodes::OplogOutOfOrder) {
        ...
        if (bgSyncHangAfterRunRollback.shouldFail()) {
            LOGV2(22734, "bgSyncHangAfterRunRollback failpoint is set.");
...
void BackgroundSync::_runRollback(OperationContext* opCtx,
    ...
    if (_replCoord->getMemberState().primary()) {
        ...
        if (!status.isOK()) {
            LOGV2_DEBUG(
                22736, 1, "Aborting catch-up failed with status: {status}", "status"_attr = status);
        }
        return;
    }
    ...
    if (!forceRollbackViaRefetch.load() && storageEngine->supportsRecoverToStableTimestamp()) {
        LOGV2(22741, "Rollback using 'recoverToStableTimestamp' method.");
        _runRollbackViaRecoverToCheckpoint(
            opCtx, source, &localOplog, storageInterface, getConnection);
    } else {
        LOGV2(22742, "Rollback using the 'rollbackViaRefetch' method.");
...
void BackgroundSync::_runRollbackViaRecoverToCheckpoint(
    ...
    LOGV2(22743, "Scheduling rollback (sync source: {source})", "source"_attr = source);
    auto status = _rollback->runRollback(opCtx);
    if (status.isOK()) {
        LOGV2(22744, "Rollback successful.");
    } else if (status == ErrorCodes::UnrecoverableRollbackError) {
        LOGV2_FATAL(
            22767, "Rollback failed with unrecoverable error: {status}", "status"_attr = status);
        fassertFailedWithStatusNoTrace(50666, status);
    } else {
        LOGV2_WARNING(
            22763, "Rollback failed with retryable error: {status}", "status"_attr = status);
    }
}
...
void BackgroundSync::stop(bool resetLastFetchedOptime) {
    ...
    _state = ProducerState::Stopped;
    LOGV2(22746, "Stopping replication producer");

    _syncSourceHost = HostAndPort();
    if (resetLastFetchedOptime) {
        invariant(_oplogApplier->getBuffer()->isEmpty());
        _lastOpTimeFetched = OpTime();
        LOGV2(22747, "Resetting last fetched optimes in bgsync");
    }
...
void BackgroundSync::start(OperationContext* opCtx) {
    ...
    do {
        ...
        if (!_oplogApplier->getBuffer()->isEmpty()) {
            LOGV2(22748, "going to start syncing, but buffer is not empty");
        }
...
bool BackgroundSync::shouldStopFetching() const {
    ...
    if (getState() != ProducerState::Running) {
        LOGV2_DEBUG(22752, 2, "Stopping oplog fetcher due to stop request.");
        return true;
    }
