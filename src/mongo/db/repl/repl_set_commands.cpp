...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        LOGV2(23210, "replSetTest command received: {cmdObj}", "cmdObj"_attr = cmdObj.toString());
...
void parseReplSetSeedList(ReplicationCoordinatorExternalState* externalState,
    ...
    while (1) {
        ...
        if (externalState->isSelf(m, getGlobalServiceContext())) {
            LOGV2_DEBUG(23213, 1, "ignoring seed {m} (=self)", "m"_attr = m.toString());
        } else {
            seeds->push_back(m);
        }
        if (*comma == 0) {
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        if (configObj.isEmpty()) {
            ...
            LOGV2(23214, "initiate : {noConfigMessage}", "noConfigMessage"_attr = noConfigMessage);
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        LOGV2(23216, "Attempting to step down in response to replSetStepDown command");

        ReplicationCoordinator::get(opCtx)->stepDown(
            opCtx, force, Seconds(secondaryCatchUpPeriodSecs), Seconds(stepDownForSecs));

        LOGV2(23217, "replSetStepDown command completed");
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        LOGV2(23218, "Received replSetStepUp request");
...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        Status status = ReplicationCoordinator::get(opCtx)->checkReplEnabledForCommand(&result);
        uassertStatusOK(status);
        LOGV2(23220, "Received replSetAbortPrimaryCatchUp request");
