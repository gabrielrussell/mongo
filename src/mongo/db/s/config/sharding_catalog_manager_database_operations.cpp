...
DatabaseType ShardingCatalogManager::createDatabase(OperationContext* opCtx,
    ...
    LOGV2(22006, "Registering new database {db} in sharding catalog", "db"_attr = db);
...
void ShardingCatalogManager::enableSharding(OperationContext* opCtx,
    ...
    LOGV2(22007, "Enabling sharding for database [{dbName}] in config db", "dbName"_attr = dbName);
