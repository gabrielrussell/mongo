...
void ShardingCatalogManager::dropCollection(OperationContext* opCtx, const NamespaceString& nss) {
    ...
    LOGV2_DEBUG(21992, 1, "dropCollection {nss_ns} started", "nss_ns"_attr = nss.ns());

    sendDropCollectionToAllShards(opCtx, nss);

    LOGV2_DEBUG(21993, 1, "dropCollection {nss_ns} shard data deleted", "nss_ns"_attr = nss.ns());

    removeChunksAndTagsForDroppedCollection(opCtx, nss);

    LOGV2_DEBUG(
        21994, 1, "dropCollection {nss_ns} chunk and tag data deleted", "nss_ns"_attr = nss.ns());
    CollectionType coll;
    coll.setNs(nss);
    ...
    LOGV2_DEBUG(
        21995, 1, "dropCollection {nss_ns} collection marked as dropped", "nss_ns"_attr = nss.ns());

    sendSSVToAllShards(opCtx, nss);

    LOGV2_DEBUG(21996, 1, "dropCollection {nss_ns} completed", "nss_ns"_attr = nss.ns());
...
void ShardingCatalogManager::generateUUIDsForExistingShardedCollections(OperationContext* opCtx) {
    ...
    if (shardedColls.empty()) {
        LOGV2(21998, "all sharded collections already have UUIDs");
...
void ShardingCatalogManager::refineCollectionShardKey(OperationContext* opCtx,
...

    {
        ...
        if (MONGO_unlikely(hangRefineCollectionShardKeyBeforeUpdatingChunks.shouldFail())) {
            LOGV2(22002, "Hit hangRefineCollectionShardKeyBeforeUpdatingChunks failpoint");
        ...
        if (MONGO_unlikely(hangRefineCollectionShardKeyBeforeCommit.shouldFail())) {
            LOGV2(22005, "Hit hangRefineCollectionShardKeyBeforeCommit failpoint");
