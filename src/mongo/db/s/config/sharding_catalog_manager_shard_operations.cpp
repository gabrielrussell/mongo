...
RemoveShardProgress ShardingCatalogManager::removeShard(OperationContext* opCtx,
    ...
    if (!isShardCurrentlyDraining) {
        LOGV2(22013, "going to start draining shard: {name}", "name"_attr = name);
    ...
    if (chunkCount > 0 || databaseCount > 0) {
        ...
        LOGV2(22014, "chunkCount: {chunkCount}", "chunkCount"_attr = chunkCount);
        LOGV2(22015, "databaseCount: {databaseCount}", "databaseCount"_attr = databaseCount);
        LOGV2(22016, "jumboCount: {jumboCount}", "jumboCount"_attr = jumboCount);
    ...
    LOGV2(22017, "going to remove shard: {name}", "name"_attr = name);
