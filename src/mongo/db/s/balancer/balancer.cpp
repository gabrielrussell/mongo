...
void warnOnMultiVersion(const vector<ClusterStatistics::ShardStatistics>& clusterStats) {
    ...
    LOGV2_WARNING(21943, "{sb_str}", "sb_str"_attr = sb.str());
}
...
void Balancer::_mainThread() {
    ON_BLOCK_EXIT([this] {
        ...
        LOGV2_DEBUG(21923, 1, "Balancer thread terminated");
    });
    ...
    LOGV2(21924, "CSRS balancer is starting");
    ...
    LOGV2(21925, "CSRS balancer thread is recovering");

    _migrationManager.finishRecovery(opCtx.get(),
                                     balancerConfig->getMaxChunkSizeBytes(),
                                     balancerConfig->getSecondaryThrottle());

    LOGV2(21926, "CSRS balancer thread is recovered");
    while (!_stopRequested()) {
        ...
        try {
            ...
            if (!balancerConfig->shouldBalance()) {
                LOGV2_DEBUG(21927, 1, "Skipping balancing round because balancing is disabled");
                _endRound(opCtx.get(), kBalanceRoundDefaultInterval);
                continue;
            }

            {
                ...
                if (!status.isOK()) {
                    ...
                    LOGV2_DEBUG(21929, 1, "Done enforcing tag range boundaries.");
                }
                ...
                if (candidateChunks.empty()) {
                    LOGV2_DEBUG(21930, 1, "no need to move any chunk");
                    _balancedLastTime = 0;
                } else {
                ...
                LOGV2_DEBUG(21931, 1, "*** End of balancing round");
            }
            ...
            LOGV2(
                21933, "caught exception while doing balance: {e_what}", "e_what"_attr = e.what());
            LOGV2_DEBUG(21934, 1, "*** End of balancing round");
    ...
    LOGV2(21935, "CSRS balancer is now stopped");
}
...
bool Balancer::_checkOIDs(OperationContext* opCtx) {
    ...
    for (const ShardId& shardId : all) {
        ...
        if (f["oidMachine"].isNumber()) {
            ...
            LOGV2(21937, "warning: oidMachine not set on: {s}", "s"_attr = s->toString());
        }
    }
...
int Balancer::_moveChunks(OperationContext* opCtx,
    ...
    if (_stopRequested() || !balancerConfig->shouldBalance()) {
        LOGV2_DEBUG(21938, 1, "Skipping balancing round because balancer was stopped");
        return 0;
    }
...
void Balancer::_splitOrMarkJumbo(OperationContext* opCtx,
    ...
    try {
        ...
        if (splitPoints.empty()) {
            LOGV2(
                21941, "Marking chunk {chunk} as jumbo.", "chunk"_attr = redact(chunk.toString()));
            chunk.markAsJumbo();
