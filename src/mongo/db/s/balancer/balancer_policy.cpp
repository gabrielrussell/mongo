...
vector<MigrateInfo> BalancerPolicy::balance(const ShardStatisticsVector& shardStats,
    ...
    if (MONGO_unlikely(balancerShouldReturnRandomMigrations.shouldFail()) &&
        ...
        LOGV2_DEBUG(21949, 1, "balancerShouldReturnRandomMigrations failpoint is set");
...
bool BalancerPolicy::_singleZoneBalance(const ShardStatisticsVector& shardStats,
    ...
    if (!to.isValid()) {
        if (migrations->empty()) {
            LOGV2(21950, "No available shards to take chunks for zone [{tag}]", "tag"_attr = tag);
        }
        return false;
    }
    ...
    LOGV2_DEBUG(21952, 1, "zone       : {tag}", "tag"_attr = tag);
    LOGV2_DEBUG(
        21953, 1, "donor      : {from} chunks on {max}", "from"_attr = from, "max"_attr = max);
    LOGV2_DEBUG(21954, 1, "receiver   : {to} chunks on {min}", "to"_attr = to, "min"_attr = min);
    LOGV2_DEBUG(21955,
                1,
