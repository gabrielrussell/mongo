...
CleanupResult cleanupOrphanedData(OperationContext* opCtx,
        ...
        if (!startingFromKey.isEmpty()) {
            if (!metadata->isValidKey(startingFromKey)) {
                *errMsg = str::stream()
                ...
                LOGV2(21980, "{errMsg}", "errMsg"_attr = *errMsg);
    ...
    LOGV2_DEBUG(
        21983, 1, "Finished waiting for last {ns} orphan range cleanup", "ns"_attr = ns.toString());

    if (!result.isOK()) {
        LOGV2(21984, "{result_reason}", "result_reason"_attr = redact(result.reason()));
