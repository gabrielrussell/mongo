...
void MigrationDestinationManager::_setStateFailWarn(StringData msg) {
    LOGV2_WARNING(22048, "{msg}", "msg"_attr = msg);
    {
        stdx::lock_guard<Latch> sl(_mutex);
        _errmsg = msg.toString();
        _state = FAIL;
...
void MigrationDestinationManager::_migrateDriver(OperationContext* outerOpCtx) {
    ...
    if (initialState == ABORT) {
        LOGV2_ERROR(22051, "Migration abort requested before it started");
        return;
    }
...

    {
        ...
        while (true) {
            ...
            for (i = 0; i < maxIterations; i++) {
                ...
                if (i > 100) {
                    LOGV2(22041, "secondaries having hard time keeping up with migrate");
                }
...

    {
        ...
        LOGV2(22042, "Waiting for replication to catch up before entering critical section");
        ...
        LOGV2(22043, "Chunk data replicated successfully.");
    }

    {
        ...
        while (getState() == STEADY || getState() == COMMIT_START) {
            ...
            if (getState() == ABORT) {
                LOGV2(22044, "Migration aborted while transferring mods");
                return;
            }
    ...
    if (xfer["reload"].isABSONObj()) {
        ...
        while (i.more()) {
            ...
            if (willOverrideLocalId(opCtx,
                ...
                LOGV2_WARNING(22050, "{errMsg}", "errMsg"_attr = errMsg);
