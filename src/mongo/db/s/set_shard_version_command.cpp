    ...
    bool errmsgRun(OperationContext* opCtx,
            ...
            if (isDropRequested) {
                ...
                if (requestedVersion < collectionShardVersion &&
                    ...
                    if (critSecSignal) {
                        collLock.reset();
                        autoDb.reset();
                        LOGV2(22093, "waiting till out of critical section");
                ...
                if (!collectionShardVersion.isSet() && !authoritative) {
                    ...
                    if (critSecSignal) {
                        collLock.reset();
                        autoDb.reset();
                        LOGV2(22094, "waiting till out of critical section");
                        critSecSignal->waitFor(opCtx, Seconds(10));
                    }
...

        {
            ...
            if (!status.isOK()) {
                ...
                LOGV2_WARNING(22095, "{errmsg}", "errmsg"_attr = errmsg);
                ...
                if (sampler.tick()) {
                    LOGV2_WARNING(22096, "{errmsg}", "errmsg"_attr = errmsg);
                }
