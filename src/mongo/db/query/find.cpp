...
Message getMore(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(23911, 5, "Running getMore, cursorid: {cursorid}", "cursorid"_attr = cursorid);
    ...
    *isCursorAuthorized = true;
    ...
    LOGV2_DEBUG(
        23914, 5, "getMore returned {numResults} results\n", "numResults"_attr = numResults);
    return Message(bb.release());
}
...
bool runQuery(OperationContext* opCtx,
    ...
    LOGV2_DEBUG(23915, 5, "Running query:\n{cq}", "cq"_attr = redact(cq->toString()));
    LOGV2_DEBUG(
        23916, 2, "Running query: {cq_Short}", "cq_Short"_attr = redact(cq->toStringShort()));
