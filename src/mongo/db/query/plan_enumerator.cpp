...
PlanEnumerator::MemoID PlanEnumerator::memoIDForNode(MatchExpression* node) {
    ...
    if (_nodeToId.end() == it) {
        LOGV2_ERROR(23947, "Trying to look up memo entry for node, none found.");
        MONGO_UNREACHABLE;
    }
...
unique_ptr<MatchExpression> PlanEnumerator::getNext() {
    ...
    _root->resetTag();
    LOGV2_DEBUG(
        23945, 5, "Enumerator: memo just before moving:\n{dumpMemo}", "dumpMemo"_attr = dumpMemo());
...
void PlanEnumerator::tagMemo(size_t id) {
    LOGV2_DEBUG(23946, 5, "Tagging memoID {id}", "id"_attr = id);
