...
Status restoreMissingFeatureCompatibilityVersionDocument(OperationContext* opCtx,
    ...
    if (!db) {
        LOGV2(20993, "Re-creating admin database that was dropped.");
    }
...
void rebuildIndexes(OperationContext* opCtx, StorageEngine* storageEngine) {
    ...
    if (!reconcileResult.indexesToRebuild.empty() && serverGlobalParams.indexBuildRetry) {
        LOGV2(20997, "note: restart the server with --noIndexBuildRetry to skip index rebuilds");
    }

    if (!serverGlobalParams.indexBuildRetry) {
        LOGV2(20998, "  not rebuilding interrupted indexes");
        return;
    }
    ...
    if (getReplSetMemberInStandaloneMode(opCtx->getServiceContext())) {
        LOGV2(21000, "Not restarting unfinished index builds because we are in standalone mode");
        return;
    }
...
bool repairDatabasesAndCheckVersion(OperationContext* opCtx) {
    ...
    if (storageGlobalParams.repair) {
        ...
        if (MONGO_unlikely(exitBeforeDataRepair.shouldFail())) {
            LOGV2(21001, "Exiting because 'exitBeforeDataRepair' fail point was set.");
        ...
        for (const auto& dbName : dbNames) {
            LOGV2_DEBUG(21002, 1, "    Repairing database: {dbName}", "dbName"_attr = dbName);
    ...
    if (storageGlobalParams.repair) {
        if (MONGO_unlikely(exitBeforeRepairInvalidatesConfig.shouldFail())) {
            LOGV2(21003, "Exiting because 'exitBeforeRepairInvalidatesConfig' fail point was set.");
            quickExit(EXIT_ABRUPT);
        }
        ...
        if (repairObserver->getModifications().size() > 0) {
            LOGV2_WARNING(21013, "Modifications made by repair:");
    ...
    for (const auto& dbName : dbNames) {
        ...
        LOGV2_DEBUG(21005, 1, "    Recovering database: {dbName}", "dbName"_attr = dbName);
    ...
    if (!fcvDocumentExists && nonLocalDatabases && !needInitialSync) {
        ...
        LOGV2_FATAL(21021, "Please run with --repair to restore the document.");
        fassertFailedNoTrace(40652);
    }

    LOGV2_DEBUG(21012, 1, "done repairDatabases");
