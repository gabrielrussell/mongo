...
MONGO_INITIALIZER(R2CellUnion_Test)(InitializerContext* context) {
    ...
    generator.seed(seed);
    LOGV2(23887, "R2CellUnion Test - Random Number Generator Seed: {seed}", "seed"_attr = seed);
    return Status::OK();
}
...
void checkCellIdCovering(const GeoHashConverter& converter,
    ...
    if (region.fastContains(cell)) {
        LOGV2(23888, "covering {covering}", "covering"_attr = covering.toString());
        LOGV2(23889, "cellId {cellId}", "cellId"_attr = cellId);
    }
