...
MinVisibleTimestampMap closeCatalog(OperationContext* opCtx) {
    ...
    LOGV2_DEBUG(21140, 1, "closeCatalog: closing collection catalog");
    LOGV2(21141, "closeCatalog: closing all databases");
    databaseHolder->closeAll(opCtx);

    // Close the storage engine's catalog.
    LOGV2(21142, "closeCatalog: closing storage engine catalog");
...
void openCatalog(OperationContext* opCtx, const MinVisibleTimestampMap& minVisibleTimestampMap) {
    ...
    LOGV2(21143, "openCatalog: loading storage engine catalog");
    auto storageEngine = opCtx->getServiceContext()->getStorageEngine();
    storageEngine->loadCatalog(opCtx);

    LOGV2(21144, "openCatalog: reconciling catalog and idents");
    ...
    LOGV2(21146, "openCatalog: reopening all databases");
    auto databaseHolder = DatabaseHolder::get(opCtx);
    ...
    for (auto&& dbName : databasesToOpen) {
        ...
        for (auto&& collNss :
            ...
            // to the oplog.
            if (collNss.isOplog()) {
                LOGV2(21147, "openCatalog: updating cached oplog pointer");
    ...
    LOGV2(21148, "openCatalog: finished reloading collection catalog");
}
}  // namespace catalog
}  // namespace mongo
