...
void IndexCatalogImpl::_logInternalState(OperationContext* opCtx,
    ...
    LOGV2_ERROR(21236, "Ready indexes:");
    ...
    LOGV2_ERROR(21238, "In-progress indexes:");
    ...
    LOGV2_ERROR(21240, "Internal Collection Catalog Entry state:");
    ...
    LOGV2_ERROR(21243, "Ready indexes:");
    ...
    LOGV2_ERROR(21245, "Index names to drop:");
...
void IndexCatalogImpl::dropAllIndexes(OperationContext* opCtx,
    ...
    for (size_t i = 0; i < indexNamesToDrop.size(); i++) {
        ...
        invariant(desc);
        LOGV2_DEBUG(21225, 1, "\t dropAllIndexes dropping: {desc}", "desc"_attr = desc->toString());
...
int IndexCatalogImpl::numIndexesTotal(OperationContext* opCtx) const {
    ...
    if (kDebugBuild) {
        try {
            ...
            // transaction anyway.
            LOGV2(21226, " Skipping dassert check due to: {ex}", "ex"_attr = ex);
        }
    }
...
int IndexCatalogImpl::numIndexesReady(OperationContext* opCtx) const {
    ...
    if (kDebugBuild) {
        ...
        if (itIndexes.size() != completedIndexes.size()) {
            LOGV2(21227, "index catalog reports: ");
            for (const IndexDescriptor* i : itIndexes) {
                LOGV2(21228, "  index: {i}", "i"_attr = i->toString());
            }

            LOGV2(21229, "collection catalog reports: ");
            for (auto const& i : completedIndexes) {
                LOGV2(21230, "  index: {i}", "i"_attr = i);
            }
