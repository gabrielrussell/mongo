...
Status dropCollection(OperationContext* opCtx,
    ...
    if (!serverGlobalParams.quiet.load()) {
        LOGV2(21202, "CMD: drop {collectionName}", "collectionName"_attr = collectionName);
    }

    if (MONGO_unlikely(hangDropCollectionBeforeLockAcquisition.shouldFail())) {
        LOGV2(21203, "Hanging drop collection before lock acquisition while fail point is set");
