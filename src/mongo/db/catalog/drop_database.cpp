...
void _finishDropDatabase(OperationContext* opCtx,
    ...
    if (MONGO_unlikely(dropDatabaseHangBeforeInMemoryDrop.shouldFail())) {
        LOGV2(21204, "dropDatabase - fail point dropDatabaseHangBeforeInMemoryDrop enabled.");
    ...
    LOGV2(21206, "dropDatabase {dbName} - finished", "dbName"_attr = dbName);
}
...
Status dropDatabase(OperationContext* opCtx, const std::string& dbName) {
...

    {
        ...
        LOGV2(21207, "dropDatabase {dbName} - starting", "dbName"_attr = dbName);
