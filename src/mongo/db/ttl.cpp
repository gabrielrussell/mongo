...
public:
    ...
    virtual void run() {
        ...
        while (!globalInShutdownDeprecated()) {
            ...
            LOGV2_DEBUG(21120, 3, "thread awake");

            if (!ttlMonitorEnabled.load()) {
                LOGV2_DEBUG(21121, 1, "disabled");
                continue;
            }
            ...
            if (lockedForWriting()) {
                ...
                LOGV2_DEBUG(21122, 3, "locked for writing");
                continue;
            }
            ...
            try {
                ...
                LOGV2_DEBUG(21123, 1, "got WriteConflictException");
...
private:
    ...
    void doTTLForIndex(OperationContext* opCtx, NamespaceString collectionNSS, BSONObj idx) {
        ...
        if (MONGO_unlikely(hangTTLMonitorWithLock.shouldFail())) {
            LOGV2(21126, "Hanging due to hangTTLMonitorWithLock fail point");
        ...
        LOGV2_DEBUG(21128, 1, "deleted: {numDeleted}", "numDeleted"_attr = numDeleted);
    }
