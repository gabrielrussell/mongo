...
shared_ptr<ReplicaSetMonitor> ReplicaSetMonitorManager::getOrCreateMonitor(const MongoURI& uri) {
    ...
    LOGV2(20779, "Starting new replica set monitor for {uri}", "uri"_attr = uri.toString());
...
void ReplicaSetMonitorManager::removeMonitor(StringData setName) {
    ...
    if (it != _monitors.end()) {
        ...
        _monitors.erase(it);
        LOGV2(
            20780, "Removed ReplicaSetMonitor for replica set {setName}", "setName"_attr = setName);
    }
}
...
void ReplicaSetMonitorManager::shutdown() {
    ...
    if (taskExecutor) {
        LOGV2_DEBUG(20781, 1, "Shutting down task executor used for monitoring replica sets");
        taskExecutor->shutdown();
    }

    if (monitors.size()) {
        LOGV2(20782, "Dropping all ongoing scans against replica sets");
    }
