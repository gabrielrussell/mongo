...
void testValidURIFormat(URITestCase testCase) {
    LOGV2(20746, "Testing URI: {testCase_URI}", "testCase_URI"_attr = testCase.URI);
    std::string errMsg;
...
TEST(MongoURI, InvalidURIs) {
    ...
    for (size_t i = 0; i != numCases; ++i) {
        ...
        LOGV2(20747, "Testing URI: {testCase_URI}", "testCase_URI"_attr = testCase.URI);
    ...
    for (const auto& file : files) {
        ...
        for (const auto& testElement : testBson) {
            ...
            if (!valid) {
                ...
                LOGV2(20748, "Testing URI: {testCase_URI}", "testCase_URI"_attr = testCase.URI);
                auto cs_status = MongoURI::parse(testCase.URI);
                ASSERT_NOT_OK(cs_status);
            } else {
