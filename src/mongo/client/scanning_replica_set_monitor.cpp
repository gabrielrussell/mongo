...
void ScanningReplicaSetMonitor::init() {
    if (areRefreshRetriesDisabledForTest()) {
        ...
        LOGV2_WARNING(24088, "*** Not starting background refresh because refresh retries are disabled.");
        return;
    }
...
void ScanningReplicaSetMonitor::SetState::rescheduleRefresh(SchedulingStrategy strategy) {
    ...
    if (isDropped) {  // already removed so no need to refresh
        LOGV2_DEBUG(24070, 1, "Stopping refresh for replica set {name} because it's removed", "name"_attr = name);
        return;
    }
    ...
    LOGV2_DEBUG(24071, 1, "Next replica set scan scheduled for {nextScanTime}", "nextScanTime"_attr = nextScanTime);
    ...
    if (ErrorCodes::isShutdownError(swHandle.getStatus().code())) {
        LOGV2_DEBUG(24072, 1, "Cant schedule refresh for {name}. Executor shutdown in progress", "name"_attr = name);
        return;
    }

    if (!swHandle.isOK()) {
        LOGV2_FATAL(24092, "Can't continue refresh for replica set {name} due to {swHandle_getStatus}", "name"_attr = name, "swHandle_getStatus"_attr = redact(swHandle.getStatus()));
        fassertFailed(40140);
    }
...
void Refresher::scheduleNetworkRequests() {
    for (auto ns = getNextStep(); ns.step == NextStep::CONTACT_HOST; ns = getNextStep()) {
        ...
        if (!swHandle.isOK()) {
            LOGV2_FATAL(24093, "Can't continue scan for replica set {set_name} due to {swHandle_getStatus}", "set_name"_attr = _set->name, "swHandle_getStatus"_attr = redact(swHandle.getStatus()));
            fassertFailed(31176);
        }
...
Refresher::NextStep Refresher::getNextStep() {
    ...
    if (_scan->hostsToScan.empty()) {
        ...
        if (!_scan->foundUpMaster) {
            LOGV2_WARNING(24089, "Unable to reach primary for set {set_name}", "set_name"_attr = _set->name);
        ...
        if (_scan->foundAnyUpNodes) {
            ...
            if (nScans <= 10 || nScans % 10 == 0) {
                LOGV2(24073, "Cannot reach any nodes for set {set_name}. Please check network connectivity and the status of the set. This has happened for {set_consecutiveFailedScans} checks in a row.", "set_name"_attr = _set->name, "set_consecutiveFailedScans"_attr = _set->consecutiveFailedScans);
            }
        }
        ...
        LOGV2_DEBUG(24074, 1, "Refreshing replica set {set_name} took {scan_timer_millis}ms", "set_name"_attr = _set->name, "scan_timer_millis"_attr = _scan->timer.millis());
...
void Refresher::receivedIsMaster(const HostAndPort& from,
    ...
    if (reply.setName != _set->name) {
        if (reply.raw["isreplicaset"].trueValue()) {
            ...
            LOGV2_ERROR(24091, "replset name mismatch: expected \"{set_name}\", but remote node {from} has replset name \"{reply_setName}\", ismaster: {replyObj}", "set_name"_attr = _set->name, "from"_attr = from, "reply_setName"_attr = reply.setName, "replyObj"_attr = replyObj);
        }
...
Status Refresher::receivedIsMasterFromMaster(const HostAndPort& from, const IsMasterReply& reply) {
    ...
    if (_set->nodes.size() != reply.members.size() ||
        ...
        LOGV2_DEBUG(24075, 2, "Adjusting nodes in our view of replica set {set_name} based on master reply: {reply_raw}", "set_name"_attr = _set->name, "reply_raw"_attr = redact(reply.raw));
    ...
    if (changedHosts || changedPrimary) {
        ...
        LOGV2(24076, "Confirmed replica set for {set_name} is {set_seedConnStr}", "set_name"_attr = _set->name, "set_seedConnStr"_attr = _set->seedConnStr);
...
void IsMasterReply::parse(const BSONObj& obj) {
    try {
        ...
        ok = false;
        LOGV2(24077, "exception while parsing isMaster reply: {e_what} {obj}", "e_what"_attr = e.what(), "obj"_attr = obj);
    }
}
...
void Node::markFailed(const Status& status) {
    if (isUp) {
        LOGV2(24078, "Marking host {host} as failed{causedBy_status}", "host"_attr = host, "causedBy_status"_attr = causedBy(redact(status)));
...
bool Node::matches(const ReadPreference pref) const {
    ...
    LOGV2_DEBUG(24080, 3, "Host {host} is {isMaster_primary_not_primary}", "host"_attr = host, "isMaster_primary_not_primary"_attr = (isMaster ? "primary" : "not primary"));
...
void Node::update(const IsMasterReply& reply) {
    ...
    LOGV2_DEBUG(24081, 3, "Updating host {host} based on ismaster reply: {reply_raw}", "host"_attr = host, "reply_raw"_attr = reply.raw);
    ...
    LOGV2_DEBUG(24082, 3, "Updating {host} lastWriteDate to {reply_lastWriteDate}", "host"_attr = host, "reply_lastWriteDate"_attr = reply.lastWriteDate);
    lastWriteDate = reply.lastWriteDate;

    LOGV2_DEBUG(24083, 3, "Updating {host} opTime to {reply_opTime}", "host"_attr = host, "reply_opTime"_attr = reply.opTime);
    opTime = reply.opTime;
...
SetState::SetState(const MongoURI& uri,
    ...
    if (name.empty())
        LOGV2_WARNING(24090, "Replica set name empty, first node: {seedNodes_begin}", "seedNodes_begin"_attr = *(seedNodes.begin()));
...
Node* SetState::findOrCreateNode(const HostAndPort& host) {
    ...
    if (it == nodes.end() || it->host != host) {
        LOGV2_DEBUG(24084, 2, "Adding node {host} to our view of replica set {name}", "host"_attr = host, "name"_attr = name);
...
void SetState::updateNodeIfInNodes(const IsMasterReply& reply) {
    ...
    if (!node) {
        LOGV2_DEBUG(24085, 2, "Skipping application of ismaster reply from {reply_host} since it isn't a confirmed member of set {name}", "reply_host"_attr = reply.host, "name"_attr = name);
        return;
    }
...
void SetState::notify() {
    ...
    for (auto it = waiters.begin(); it != waiters.end();) {
        ...
        if (!match.empty()) {
            ...
            LOGV2_DEBUG(24086, 1, "Unable to statisfy read preference {it_criteria} by deadline {it_deadline}", "it_criteria"_attr = it->criteria, "it_deadline"_attr = it->deadline);
