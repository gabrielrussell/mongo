...
bool DBClientCursor::init() {
    ...
    try {
        ...
        // log msg temp?
        LOGV2(20720, "DBClientCursor::init call() failed");
        throw;
    }
    if (reply.empty()) {
        // log msg temp?
        LOGV2(20721, "DBClientCursor::init message from call() was empty");
        return false;
    }
    dataReceived(reply);
    return true;
...
bool DBClientCursor::initLazyFinish(bool& retry) {
    ...
    if (!recvStatus.isOK() || reply.empty()) {
        ...
        if (reply.empty())
            LOGV2(20723, "DBClientCursor::init message from say() was empty");
