...
Refresher::NextStep Refresher::getNextStep() {
    ...
    if (_scan->hostsToScan.empty()) {
        ...
        if (!_scan->foundUpMaster) {
            LOGV2_WARNING(
                20774, "Unable to reach primary for set {set_name}", "set_name"_attr = _set->name);
...
bool Node::matches(const ReadPreference pref) const {
    if (!isUp) {
        LOGV2_DEBUG(20764, 3, "Host {host} is not up", "host"_attr = host);
        return false;
    }
...
void SetState::notify() {
    ...
    for (auto it = waiters.begin(); it != waiters.end();) {
        ...
        if (!match.empty()) {
            ...
            LOGV2_DEBUG(20772, 1, "Unable to statisfy read preference because tests fail quickly");
            it->promise.setError(makeUnsatisfedReadPrefError(it->criteria));
            waiters.erase(it++);
        } else {
            it++;
